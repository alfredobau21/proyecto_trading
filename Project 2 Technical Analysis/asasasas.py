...
class TradingStrategy:
    ...
    def profit(self, trial):
        ...
        # Generate technical data with signals
        technical_data = self.create_signals(self.data,
                                             RSI=(rsi_window, rsi_lower_threshold),
                                             Bollinger=(bollinger_window, 2),  # Assuming std is 2
                                             MACD=(macd_fast, macd_slow, macd_sign),
                                             Stochastic=(stoch_k_window, stoch_d_window, 0),  # Assuming smoothing is 0
                                             SMA=(short_ma_window, long_ma_window))
        ...
        # Clean up remaining active positions at the end
        active_pos_copy = active_positions.copy()
        for pos in active_pos_copy:
            capital += row.Close * pos["n_shares"] * (1 - COM)
            active_positions.remove(pos)
    ...
    def backtest_strategy(self, best_params):
        ...
        technical_data = self.create_signals(self.data,
                                             ...
                                             Stochastic=self.best_params.get('Stochastic', (14, 3, 3)),
                                             SMA=self.best_params.get('SMA', (20, 50)))
