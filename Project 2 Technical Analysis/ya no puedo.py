...
class TradingStrategy:
    ...
    def optimize_strategy(self, indicators, n_trials_per_strategy=5):
        best_params = {}
        ...
        def objective(trial, indicators):
            ...
            for indicator in indicators:
                if trial.suggest_int(f'use_{indicator}', 0, 1):
                    strategy_indicators.append(indicator)
        ...
        for i in range(1, len(indicators) + 1):
            for subset in itertools.combinations(indicators, i):
                study.optimize(partial(objective, indicators=list(subset)), n_trials=n_trials_per_strategy)
                best_params[subset] = study.best_params

        self.best_params = best_params
        return best_params

    def backtest_strategy(self, best_params=None):
        if not best_params:
            best_params = self.best_params

        strategy_indicators = list(best_params.keys())[0]
        kwargs = best_params[strategy_indicators]

        backtest_data = self.data.copy()
        backtest_data = self.create_signals(backtest_data, **kwargs)
...
indicators = ['RSI', 'Bollinger', 'MACD', 'Stochastic', 'SMA']  # Indicadores disponibles
best_params = strategy.optimize_strategy(indicators)
print(best_params)
